    ...
    },
    {
      ...
      defaultValue: editItem?.examType
        ? {
            ...
            name: t(getNameById(examType, String(editItem?.examType))),
          }
        : null,
      type: 'select',
  ...
  const schoolInputs: TInput<IExamPayloadForm>[] = [
    {
      ...
      options: schoolSubjects.map((subject) => ({
        id: subject.id,
        name: `${subject.subject.name} : ${subject.grade.name}`,
      })),
      ...
      hidden: !!subjectId,
      onChange: (val) => {
        setGradeSubject(val as TAutoComplete);
